...
ner_model_bc5cdr = spacy.load('en_core_sci_sm')
...
# Split the text into chunks of 1500,000 characters
chunk_size = 1500000
...
# Separate diseases and drugs
    diseases_bc5cdr = [token[0] for token in tokens_entities_bc5cdr if token[1] == 'DISEASES']
    drugs_bc5cdr = [token[0] for token in tokens_entities_bc5cdr if token[1] == 'DRUGS']
...
output_csv_file_path = r'NEResult.csv'
